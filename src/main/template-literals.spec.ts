  ...
  it('should support string interpolation', () => {
    ...
    const friendsStr = person => {
      if (person.friends.length === 3) {
        return `${person.name} has ${person.friends.length} friends: ${person.friends[0]}, ${person.friends[1]}, ${person.friends[2]}`
      }
      return `${person.name} has ${person.friends.length} friends: ${person.friends[0]}, ${person.friends[1]}, ${person.friends[2]}, ${person.friends[3]}`
    }
    ...
    it('should call the tagging function', () => {
      ...
      function tagIt(literalString, ...interpolatedParts) {
        ...
        return `${literalString[0]} dear ${interpolatedParts[0]}  ${literalString[1]}   ${interpolatedParts[1]}  ${literalString[2]}` ;
      }
    })
    ...
    it('can be curried', () => {
      ...
      let journey = (...arg1) => (...arg2) => (...arg3) => {
        return `${arg1}, then ${arg2} and finally ${arg3}!`;
      }
